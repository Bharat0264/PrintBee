...
export async function POST(request: Request) {
  ...
  if (title.length < 3 || title.length > 140 || !categories.has(category) || shortDescription.length < 20 || shortDescription.length > 360 || !Number.isFinite(requested) || requested < 0 || requested > 1_000_000 || body.ownershipConfirmed !== true) return NextResponse.json({ error: "Complete the required project details and ownership declaration" }, { status: 400 });
  const id = crypto.randomUUID(); const now = new Date().toISOString(); const base = slugify(title) || "project"; let slug = base; let suffix = 2;
