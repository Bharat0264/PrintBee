...
export async function POST(request: Request) {
  ...
  await database().prepare("INSERT INTO print_services (id,name,description,active,is_binding,counts_for_packaging,price_paise,created_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,description=excluded.description,is_binding=excluded.is_binding,counts_for_packaging=excluded.counts_for_packaging,price_paise=excluded.price_paise,active=1")
    .bind(id, name, body.description?.trim().slice(0, 125) ?? "", 1, body.isBinding ? 1 : 0, body.countsForPackaging === false ? 0 : 1, pricePaise, new Date().toISOString()).run();
