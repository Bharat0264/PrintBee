...
export async function GET(_request: Request, context: { params: Promise<{ uploadId: string }> }) {
  ...
  const file = await database().prepare("SELECT original_name, content_type, storage_key, deleted_at FROM uploads WHERE id=? AND order_id IS NOT NULL").bind(uploadId).first<{ original_name: string; content_type: string; storage_key: string; deleted_at: string | null }>();
