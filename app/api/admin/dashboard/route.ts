...
export async function GET() {
  ...
  const [summary, orders, hiddenOrders, revenueOrders, activeUsers, riders, riderApplications, locations, locationStats, files, riderPayments, riderWithdrawals] = await Promise.all([
    db.prepare(`SELECT COUNT(*) total, SUM(CASE WHEN status='DELIVERED' THEN 1 ELSE 0 END) delivered, SUM(CASE WHEN payment_status='PAID' THEN 1 ELSE 0 END) paid, SUM(CASE WHEN payment_status!='PAID' THEN 1 ELSE 0 END) unpaid, SUM(CASE WHEN payment_status='PAID' THEN total_paise ELSE 0 END) revenue_paise, SUM(CASE WHEN status='READY_FOR_PICKUP' THEN 1 ELSE 0 END) ready FROM orders WHERE hidden_at IS NULL`).first(),
    db.prepare(`SELECT id, order_number, customer_email, customer_name, mobile_number, location_name, items_json, printing_subtotal_paise, delivery_fee_paise, platform_fee_paise, total_paise, payment_status, payment_reference, payment_rejection_reason, payment_verified_at, payment_verified_by, payment_qr_storage_key IS NOT NULL has_payment_qr, payment_qr_file_name, payment_qr_deleted_at, status, rider_email, cancellation_reason, cancelled_at, cancelled_by, delivered_at, created_at FROM orders WHERE hidden_at IS NULL ORDER BY created_at DESC LIMIT 100`).all(),
