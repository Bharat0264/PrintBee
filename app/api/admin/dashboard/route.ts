...
export async function GET() {
  ...
  const db = database();
  const [summary, orders, riders, riderApplications, locations, locationStats, files, riderPayments] = await Promise.all([
    ...
    db.prepare(`SELECT id, order_number, customer_email, customer_name, mobile_number, location_name, items_json, printing_subtotal_paise, delivery_fee_paise, platform_fee_paise, total_paise, payment_status, payment_reference, payment_rejection_reason, payment_verified_at, status, rider_email, cancellation_reason, cancelled_at, cancelled_by, created_at FROM orders ORDER BY created_at DESC LIMIT 100`).all(),
