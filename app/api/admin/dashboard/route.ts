...
export async function GET() {
  ...
  const [summary, orders, riders, locations, files] = await Promise.all([
    ...
    db.prepare(`SELECT id, order_number, customer_name, location_name, items_json, total_paise, payment_status, payment_reference, status, rider_email, created_at FROM orders ORDER BY created_at DESC LIMIT 50`).all(),
  ...
  return NextResponse.json({
    summary,
    orders: (orders.results as Array<{ id: string }>).map((order) => ({ ...order, files: filesByOrder.get(order.id) ?? [] })),
