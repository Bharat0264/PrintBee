...
export async function POST(request: Request) {
  ...
  const order = await database().prepare("SELECT id, delivery_code_hash, status, payment_status, rider_email, payment_qr_storage_key FROM orders WHERE order_number = ?").bind(orderNumber?.trim().toUpperCase()).first<{ id: string; delivery_code_hash: string; status: string; payment_status: string; rider_email: string | null; payment_qr_storage_key: string | null }>();
  ...
  const updated = await database().prepare("UPDATE orders SET status='DELIVERED', delivered_at=?, delivered_by=?, payment_qr_storage_key=NULL, payment_qr_deleted_at=? WHERE id=? AND status='RIDER_ASSIGNED'").bind(now, viewer.email, now, order.id).run();
