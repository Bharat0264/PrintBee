...
export async function POST(request: Request) {
  ...
  await database().prepare("INSERT INTO app_users (email, role, created_at) VALUES (?, 'AGENT', ?) ON CONFLICT(email) DO UPDATE SET role = 'AGENT'").bind(cleanEmail, new Date().toISOString()).run();
