...
export async function GET() {
  ...
  return NextResponse.json({ role: viewer.isAdmin ? "ADMIN" : row?.role ?? "CUSTOMER", isFranchise: viewer.isFranchise, adminRole: viewer.adminRole ?? null, approvalStatus: row?.approval_status ?? null, isAvailable: Boolean(row?.is_available), referralCode: profile?.referral_code, pointsBalance: profile?.points_balance ?? 10, hasReferrer: Boolean(profile?.referred_by_email), latestCredit });
}
