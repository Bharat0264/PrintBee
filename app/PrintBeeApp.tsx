          ...
          {batchFiles.length > 0 && (() => {
            ...
            return <section className="binding-fields batch-file-review" aria-labelledby="batch-file-review-title">
              ...
              <article className="batch-file-card" onTouchStart={(event) => setSwipeStartX(event.changedTouches[0]?.clientX ?? null)} onTouchEnd={(event) => { const endX = event.changedTouches[0]?.clientX; if (swipeStartX !== null && endX !== undefined && Math.abs(endX - swipeStartX) > 45) moveBatch(endX < swipeStartX ? 1 : -1); setSwipeStartX(null); }}>
                ...
                <div className="batch-file-choice-cards"><div className="batch-choice-group"><small>Service</small><div className="service-option-grid" role="radiogroup" aria-label="Print service">{printServices.map((service) => <button type="button" role="radio" aria-checked={item.serviceId === service.id} className={item.serviceId === service.id ? "selected" : ""} key={service.id} onClick={() => updateBatchFile(batchIndex, { serviceId: service.id })}><span><strong>{service.name}</strong><small>{service.description}</small></span><b>{service.price_paise ? `+${inr.format(service.price_paise / 100)}` : "Included"}</b></button>)}</div></div><div className="batch-choice-group"><small>Print style</small><div className="option-grid" role="radiogroup" aria-label="Print style">{options.map((option) => <button type="button" role="radio" aria-checked={item.mode === option.id} className={`print-option ${item.mode === option.id ? "selected" : ""}`} key={option.id} onClick={() => updateBatchFile(batchIndex, { mode: option.id })}><span className={`mode-icon ${option.id.startsWith("colour") ? "colour" : ""}`}>{option.icon}</span><span><strong>{option.title}</strong><small>{option.note}</small></span></button>)}</div></div><div className="batch-copies"><small>Copies</small><div className="quantity-stepper"><button type="button" disabled={item.copies <= 1} onClick={() => updateBatchFile(batchIndex, { copies: Math.max(1, item.copies - 1) })}>−</button><output>{item.copies}</output><button type="button" onClick={() => updateBatchFile(batchIndex, { copies: item.copies + 1 })}>+</button></div></div></div>
