...
const ledgerFinancialColumns = [
  ...
  ["Total profit", "netProfitPaise"], ["Bharat 35% shared", "bharatSharedProfitPaise"], ["Bharat packing profit", "bharatPackingProfitPaise"], ["Admin direct profit", "adminDirectProfitPaise"], ["Bharat total", "bharatTotalProfitPaise"], ["Ramya 65% shared", "ramyaTotalProfitPaise"], ["Share tally", "shareTallyPaise"],
] as const;
              ...
              <div className="dashboard-block">
                ...
                <section className="ledger-section" id="admin-ledger">
                  ...
                  {!ledger ? <form className="ledger-lock" onSubmit={(event) => { event.preventDefault(); unlockLedger(); }}><label>Ledger password<input type="password" autoComplete="current-password" value={ledgerPassword} onChange={(event) => setLedgerPassword(event.target.value)} placeholder="Enter password" /></label><button disabled={!ledgerPassword}>Unlock ledger</button>{ledgerMessage && <p>{ledgerMessage}</p>}</form> : <>
                    ...
                    <p className="ledger-note">Every row reconciles collected amount to costs and profit. Printing, service and add-on profit is divided Bharat 35% / Ramya 65%. The full packing profit (₹1.70 per packaged order) goes to Bharat; delivery profit, platform, surge and late-night profit also remain with admin. Discounts reduce admin direct profit.</p>
