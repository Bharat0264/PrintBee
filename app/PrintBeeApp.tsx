...
const ledgerFinancialColumns = [
  ...
  ["Service/additional revenue", "serviceRevenuePaise"], ["Add-ons revenue / profit", "addonRevenuePaise"], ["Delivery collected", "deliveryCollectedPaise"], ["ADMIN50 delivery discount", "couponDeliveryDiscountPaise"], ["Delivery partner fee", "riderCostPaise"],
...
export default function PrintBeeApp({ viewer, supabaseConfig }: { viewer: Viewer; supabaseConfig: SupabaseConfig }) {
  ...
  const checkoutBeforePoints = cartTotal + checkoutDeliveryFee - checkoutCouponDeliveryDiscount + checkoutIncampusFee + checkoutPlatformFee + checkoutPackagingFee + checkoutSurgeFee + checkoutLateNightFee + checkoutGatewayFee;
                ...
                <div className="admin-orders">{visibleAdminOrders.length ? visibleAdminOrders.map((order: any) => (
                  <article className={expandedAdminOrders[order.id] ? "is-expanded" : "is-compact"} key={order.id}>
                    <div className="order-customer">
                      ...
                      {order.coupon_code && <small><b>Coupon used:</b> {order.coupon_code} · Delivery discount {inr.format((order.coupon_delivery_discount_paise ?? 0) / 100)}</small>}
                    ...
                    <div className="document-details compact-order-details">
                      ...
                      <div className="order-bill-summary"><strong>Bill summary</strong><span><small>Printing &amp; products</small><b>{inr.format(order.printing_subtotal_paise / 100)}</b></span><span><small>Delivery</small><b>{inr.format(order.delivery_fee_paise / 100)}</b></span>{(order.coupon_delivery_discount_paise ?? 0) > 0 && <span><small>{order.coupon_code} delivery discount</small><b>−{inr.format(order.coupon_delivery_discount_paise / 100)}</b></span>}{(order.incampus_fee_paise ?? 0) > 0 && <span><small>In-campus delivery</small><b>{inr.format(order.incampus_fee_paise / 100)}</b></span>}<span><small>Platform fee</small><b>{inr.format(order.platform_fee_paise / 100)}</b></span>{(order.packaging_fee_paise ?? 0) > 0 && <span><small>Packaging</small><b>{inr.format(order.packaging_fee_paise / 100)}</b></span>}{(order.payment_gateway_fee_paise ?? 0) > 0 && <span><small>Payment fee</small><b>{inr.format(order.payment_gateway_fee_paise / 100)}</b></span>}{(order.surge_fee_paise ?? 0) > 0 && <span><small>Surge charge</small><b>{inr.format(order.surge_fee_paise / 100)}</b></span>}{(order.late_night_fee_paise ?? 0) > 0 && <span><small>Late-night delivery</small><b>{inr.format(order.late_night_fee_paise / 100)}</b></span>}<span className="bill-total"><small>Total</small><b>{inr.format(order.total_paise / 100)}</b></span></div>
                    </div>
      ...
      {safariUpiNoticeOpen && (
        <div className="modal-backdrop browser-payment-backdrop" role="presentation" onMouseDown={() => setSafariUpiNoticeOpen(false)}>
          <section className="checkout-modal browser-payment-notice" role="dialog" aria-modal="true" aria-labelledby="safari-upi-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <button className="save-button" onClick={() => { window.location.href = "googlechromes://www.printbee.co.in"; }}>Open in Chrome</button>
